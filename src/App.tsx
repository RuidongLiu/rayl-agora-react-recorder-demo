...
function App() {
  ...
  const channel = useRef("52");
  // you can apply appid follow the guide https://www.agora.io/en/blog/how-to-get-started-with-agora/
  const appid = useRef("82896ffe7f99469095a03a3cb1968d7f");
  // you can apply token follow the guide https://www.agora.io/en/blog/how-to-get-started-with-agora/
  const token = useRef("007eJxTYOB/d2bOTP55evpiiYGSM3Ze+rzFf0/CyvTVW+82qesfz5mowGBhZGFplpaWap5maWliZmlgaZpoYJxonJxkaGlmkWKeFvLgQlpDICPDx7IfzIwMEAjiMzGYGjEwAAA0UR+t");
  ...
  const onUserPublish = async (
    ...
    if (mediaType === "video") {
      ...
      // record remote video for 10s
      const stopRecording = startRecording(remoteTrack);
      console.log("start recording remote video for 10s");
      setTimeout(async () => {
        stopRecording();
      }, 10_000);
